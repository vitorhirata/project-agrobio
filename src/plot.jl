...
if mode == "standard"
    p = plot(df, x=:meanDU, y=:nVar, Geom.line, Guide.ylabel("Numero de Variedades"), Guide.ylabel("Numero Medio de Variedades por UD"))
...
elseif mode == "varietyDistribution"
    ...
    elseif length(split(input_file, "_")) == 3
        p = plot(df, x=:value, y=:varDist, color=:param, Geom.line, Scale.color_discrete(), Guide.colorkey(param), Coord.cartesian(xmin=0, xmax=20, ymin=0, ymax=1))
        draw(SVG(output_file, 15cm, 10cm), p)
        println("Image $(output_file) successfully generated.")
    end
...
elseif mode == "histogramFitnessVar"
    p = plot(df, x=:value, y=:fitness, color=:param, Geom.line, Scale.color_discrete(), Guide.colorkey(param), Scale.y_continuous(minvalue=0, maxvalue=0.2))
    draw(SVG(output_file, 15cm, 10cm), p)
    println("Image $(output_file) successfully generated.")
    p2 = plot(df, x=:value, y=:appearence, color=:param, Geom.line, Scale.color_discrete(), Guide.colorkey(param), Scale.y_continuous(minvalue=0, maxvalue=0.1))
    output_file2 = input_file[1:end-4] * "2.svg"
    draw(SVG(output_file2, 15cm, 10cm), p2)
    println("Image $(output_file2) successfully generated.")
